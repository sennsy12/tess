...
import type { Order, OrderDetail, OrderLine, OrderStatusHistoryEntry } from '../../types/order';
...
export const ordersApi = {
  ...
  getOne: (ordrenr: number) => api.get<OrderDetail>(`/orders/${ordrenr}`),
  searchByReferences: (q: string) =>
    api.get<OrderSearchResult>('/orders/search/references', { params: { q } }),
  updateStatus: (ordrenr: number, workflowStatus: string, comment?: string) =>
    api.patch<Order>(`/orders/${ordrenr}/status`, comment ? { workflowStatus, comment } : { workflowStatus }),
  /** Workflow timeline: who/when/from→to/comment (kunde-scoped on the server). */
  getHistory: (ordrenr: number) => api.get<{ data: OrderStatusHistoryEntry[] }>(`/orders/${ordrenr}/history`),
  ...
  cancel: (ordrenr: number) => api.patch<Order>(`/orders/${ordrenr}/cancel`),
};
...
export const orderlinesApi = {
  getByOrder: (ordrenr: number, params?: { page?: number; limit?: number }) =>
    api.get<PaginatedResponse<OrderLine>>(`/orderlines/order/${ordrenr}`, { params }),
