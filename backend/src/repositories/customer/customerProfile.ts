/**
 * Single round-trip customer profile query (company, group, portal user,
 * order stats, contact refs). Moved verbatim from `models/customerModel`
 * so the model file stays a thin facade.
 *
 * @module repositories/customer/customerProfile
 */
import { query } from '../../db/index.js';
import { SQL_ACTIVE_ORDER_WHERE } from '../../lib/orderWorkflow.js';
import type { CustomerProfileDbRow } from '../../models/customerModel.js';
...
export async function findCustomerProfile(
  kundenr: string,
): Promise<CustomerProfileDbRow | null> {
